...
В нем меню реализовано двумя отдельными классами  TMenuThemes и TPopuMenuThemes.
Параметры конструктора класса TMenuThemes.Create(MainMenu: TMainMenu; index: SmallInt);
**MainMenu: TMainMenu** - это компонент главного меню, который должен быть на форме
**index** - задает позицию в порядке перечисления в главном меню куда будет вставлено созданное меню. 
Например, раздел "File" идет всегда первым и имеет index = 0, если при создании меню задать index = 1, то раздел Themes будет создано вторым по счету так:
"File" **"Themes"** "Help" 
